...
const getAmount = (plan) => {
  if (plan === 'Basic') return 4839900; // ₹48,399
  if (plan === 'Standard') return 6039900; // ₹60,399
  return 7539900; // ₹75,399 for Premium
};
...
const IOTMasterPackage = () => {
  ...
  const [finalAmount, setFinalAmount] = useState(getAmount('Basic'));
  ...
  return (
    <div className="iot-master-container">
      ...
      <div className="pricing-box">
        ...
        <p>₹48,399 (Basic), ₹60,399 (Standard), ₹75,399 (Premium)</p>
      </div>
        ...
        >
          ...
          <label style={{ fontWeight: 'bold' }}>
            ...
            <select name="plan" value={formData.plan} onChange={handleInputChange} style={{ marginLeft: 8, padding: 6, borderRadius: 5, border: '1px solid #ccc', width: '80%' }}>
              <option value="Basic">Basic (₹48,399)</option>
              <option value="Standard">Standard (₹60,399)</option>
              <option value="Premium">Premium (₹75,399)</option>
            </select>
          </label>
          <label style={{ fontWeight: 'bold' }}>
            Coupon Code:
